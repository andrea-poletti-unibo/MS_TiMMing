...
com <- fread("results/JB_paper/def_aug23/CoMMpass_20boots/CoMMpass_timings_20boot.txt")
...
smm <- fread("results/JB_paper/def_aug23/SU2C&BUSnew_20boots/SU2C&BUSnew_timings_20boot.txt")
...
#_________ Bland-Altman plots ____________


ba <- m %>% select(cna, med_timing_com, med_timing_smm) %>% mutate(difference = med_timing_com - med_timing_smm, mean = (med_timing_com + med_timing_smm)/2)

mean_diff <- mean(ba$difference)

# limits of agreement
loa_upper <- mean_diff + 1.96 * sd(ba$difference)
loa_lower <- mean_diff - 1.96 * sd(ba$difference)

ba$cna <- ba$cna %>% str_remove("_chr") %>% str_remove("_del|_amp") %>% str_replace("del_","-") %>% str_replace("amp_","+")

ba %>% ggplot(aes(x = mean, y = difference)) +
  geom_point() +
  geom_hline(yintercept = mean_diff, col = "blue") +
  geom_hline(yintercept = loa_upper, col = "red", linetype = "dashed") +
  geom_hline(yintercept = loa_lower, col = "red", linetype = "dashed") +
  xlab("Mean of models") +
  ylab("Difference between models") +
  ggtitle("Bland-Altman plot of BT-scores difference (MGUS/SMM vs NDMM model)") + 
  ylim(-4, 4) + 
  annotate("text", x = 0.5, y = 0.5, label = "Mean difference", col = "blue") +
  annotate("text", x = 1, y = 3.3, label = "UPPER limit of agreement (+1.96 SD)", col = "red") +
  annotate("text", x = 1, y = -3, label = "LOWER limit of agreement (-1.96 SD)", col = "red") +
  ggrepel::geom_text_repel(data= ba %>% filter(abs(difference) > 1), aes(label = cna), size = 3) +
  theme_bw()

ggsave("plots/Bland-Altman_SMM_vs_MM_models.pdf", width = 8, height = 6, dpi = 300)
ggsave("plots/Bland-Altman_SMM_vs_MM_models.svg", width = 8, height = 6)


#____________________ VIZ 1 _________________________________

# m2 %>% filter(qval<0.05) %>% ggplot(aes(med_timing_com, med_timing_smm)) + 
...
# geom_label_repel(data= m2 %>% filter(abs(med_timing_com - med_timing_smm) > 1.5), aes(label = cna), size = 3) +
# geom_label_repel(data= m2 %>% filter(med_timing_com < 1.5 & med_timing_smm < 1.5), aes(label = cna), size = 3) +
...
dir.create("results/JB_paper/def_aug23/boot_approach", showWarnings = FALSE, recursive = TRUE)

# save plot
ggsave("results/JB_paper/def_aug23/boot_approach/compare_Viz1_SMM_vs_CoMMpass_timings_BOOTS.png", width = 7, height = 6, dpi = 300)
...
# save plot
ggsave("results/JB_paper/def_aug23/boot_approach/compare_Viz2_SMM_vs_CoMMpass_timings_BOOT.png", width = 8, height = 8, dpi = 300)
...
ggsave("results/JB_paper/def_aug23/boot_approach/combined3plots_SMM_vs_CoMMpass_timings_BOOT.pdf", plot = GR3, width = 8, height = 7, dpi = 300)


#____ SD version __________

p3sd <- ml2 %>% ggplot(aes(cna_order, timing)) +
    geom_errorbar(aes(ymin = timing - sd, ymax = timing + sd, fill = model, colour = qval_signif),
        colour = "grey50", size = 0.3, position = position_dodge(width = 0.8)
    ) +
    geom_point(position = position_dodge(width = 0.8), aes(shape=model, fill = model, colour = qval_signif), size=1.7) +
    theme_bw() +
...
GR3sd <- plot_grid(p2, p1, p3sd, ncol = 1, align = "v", rel_heights = c(1,0.8,1.5))
GR3sd

ggsave("results/JB_paper/def_aug23/boot_approach/combined3plots_SD_SMM_vs_CoMMpass_timings_BOOT.pdf", plot = GR3sd, width = 8, height = 7, dpi = 300)
...
m2 %>% ggplot(aes(timing_diff, -log(qval))) + geom_point()

m2 %>% ggplot(aes(timing_diff, -log(qval))) + geom_point(aes(colour=timing_diff)) + geom_hline(yintercept = -log(0.05), linetype=2) + geom_vline(xintercept = c(-1,1), linetype=2) +
