...
calls_0 <- fread("results/JB_paper/calls/clonalityCalls_broad_focal_CoMM-SU2Cnew.txt")

purity_file <- fread("data/CoMMpass_acs/CoMMpass_ABSOLUTE_tables_969samp.txt")
purity_0 <- purity_file %>% select(sample, purity) %>% filter(sample %in% calls_0$sample)

CI95_0 <- fread("results/JB_paper/calls/allCalls_CN_CI_broad_focal_CoMM-SU2Cnew.txt")

# save the prepared data in the workfiles
write_tsv(calls_0, "workfiles/AllTogether_model/prep_calls_NDMM.txt")
write_tsv(CI95_0, "workfiles/AllTogether_model/prep_CI95_NDMM.txt")
write_tsv(purity_0, "workfiles/AllTogether_model/prep_purity_NDMM.txt")
...
for (b in 1:20) {
    ...
    calls_b$sample <- paste0("coMMpass_", str_pad(1:NUM, 3, side = "left", pad = "0"), "_Boot_", b)
    CI95_b$ID <- paste0("coMMpass_", str_pad(1:NUM, 3, side = "left", pad = "0"), "_Boot_", b)
    purity_b$sample <- paste0("coMMpass_", str_pad(1:NUM, 3, side = "left", pad = "0"), "_Boot_", b)
    ...
    dir.create("workfiles/CoMMpass_boots/", showWarnings = FALSE, recursive = TRUE)

    # save aggregated table with points
    write_tsv(expscorestab, paste0("workfiles/CoMMpass_boots/CoMMpass_BOOT_",b,"_matches_results_ClonDiffPoints.txt"))
}

stopCluster(cl)

write_tsv(boot_data, "workfiles/CoMMpass_boots/CoMMpass_BOOT-data.txt")