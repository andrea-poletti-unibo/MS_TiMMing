...
import <- fread("workfiles/CoMM-SU2Cnew_matches_results_ClonDiffPoints.txt") 
...
gg <- ab_df %>% 
  ...
  ggtitle("Bradley-Terry CNAs timing estimates on SU2C new cohort", subtitle = "HD call and Clonal difference points" )+
...
# save plot
ggsave(plot = grid, filename =  "plots/timing_maps/CoMM-SU2Cnew_timing_map_HDcall_clonDiff.png", width = 14, height = 12, units = "in")
...
gg <- ab_df %>% filter(cna %in% hifreq) %>% 
  ...
  geom_point(aes(color=CNA_type)) + 
  xlab("CNA") + ylab("Timing Estimate") +
  ggtitle("Bradley-Terry CNAs timing estimates on SU2C new cohort", subtitle = "HD call and Clonal difference points - HighFreq > 30" )+
  coord_flip() + theme(legend.position = "bottom", plot.margin = unit(c(0.5,0,0,0.5), "cm"))
...
# save plot
ggsave(plot = grid, filename =  "plots/timing_maps/CoMM-SU2Cnew_timing_map_HiFreq_HDcall_clonDiff.png", width = 14, height = 12, units = "in")
...
write_tsv(timing_freq,"workfiles/BTmodel_CoMM-SU2Cnew_HDcall_clonDiff.txt") 
