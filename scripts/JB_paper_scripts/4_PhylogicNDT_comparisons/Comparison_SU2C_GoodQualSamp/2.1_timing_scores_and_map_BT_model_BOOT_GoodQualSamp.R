...
files <- list.files("workfiles/Phylogic_comparison", full.names = T, pattern = "matches")
...
# dir.create("results/JB_paper/def_aug23/SU2C&BUSnew_20boots/", showWarnings = F, recursive = T)
# write_tsv(res_all, "results/JB_paper/def_aug23/SU2C&BUSnew_20boots/SU2C&BUSnew_timings_20boot.txt")

# res_all <- fread("results/JB_paper/def_aug23/SU2C&BUSnew_20boots/SU2C&BUSnew_timings_20boot.txt")



res_all_boots <- res_all %>% group_by(cna) %>% summarise(med_timing=median(timing), mean_timing=mean(timing), sd = sd(timing), min= min(timing), max= max(timing), first_q= quantile(timing)[2], third_q= quantile(timing)[4],  n=n()) %>% arrange(med_timing)
...
# remove outliers
res_all2 <- res_all %>% filter(timing < 10)
res_all_boots2 <- res_all_boots %>% filter(med_timing < 10)


write_tsv(res_all, "workfiles/Phylogic_comparison/BTmodels_30BOOTS_GoodQualSamp.txt")
write_tsv(res_all_boots, "workfiles/Phylogic_comparison/Aggregate_BTmodels_30BOOTS_GoodQualSamp.txt")




p1 <- res_all2 %>% ggplot(aes(x=timing, y = cna_f)) + 
  ...
  geom_point(data = res_all_boots2, aes(x=med_timing, y = cna_f, colour=CNA_type), size=2)+
  geom_errorbar(data = res_all_boots2, aes(x=med_timing, y = cna_f, xmin= (mean_timing-(sd)), xmax=(mean_timing+(sd))), width=0.2) +
  theme_bw() +
  theme(legend.position = "bottom", plot.margin = unit(c(0.5,0,0,0.5), "cm")) +
  xlab("Timing Estimate") +
  ylab("CNA") +
  ggtitle("Bradley-Terry CNAs timing estimates on SU2C good quality samples (n=32)", subtitle = "30 bootstraps runs")

p1


dir.create("plots/timing_maps/Phylogic_comparison/", showWarnings = F, recursive = T)
# save plot
ggsave(plot = p1, filename =  "plots/timing_maps/Phylogic_comparison/GoodQualSamp_timing_map_30BOOTS.png", width = 9, height = 10, units = "in")
...
#   xlab("Timing Estimate") +
#   ylab("CNA") +
#   ggtitle("Bradley-Terry CNAs timing estimates on SU2C&BUS new cohort", subtitle = "20 bootstraps runs")
...
p2 <- res_all_hi %>% ggplot(aes(x=timing, y = cna_f)) + 
  ...
  xlab("Timing Estimate") +
  ylab("CNA") +
  ggtitle("Bradley-Terry CNAs timing estimates on SU2C&BUS new cohort", subtitle = "20 bootstraps runs") + xlim(0,6.6)

p2

# save plot
ggsave(plot = p2, filename =  "plots/timing_maps/boot_approach/SU2Cnew&BUSnew_timing_map_20BOOTS_hifreq.png", width = 14, height = 12, units = "in")
