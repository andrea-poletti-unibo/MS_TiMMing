...
#___________________________ 35 samples with high purity and good coverage ___________________________

mail_text <- "CSL203, PD47570a, CSL141, CSP65, CSL161, PD47567a, pM1835, CSL175, CSL211, pM11004, pM10157, CSL171, pM5600, PD47580a, pM9982, pM9710, pM4587, pM10109, PD47576a, CSL192, pM9660, pM5338, pM11003, CSL191, IL0291, pM10990, pM10943, pM10684, pM10619, IID_H196064_T01, CSL144, pM10114, pM5245, PD47579a, IL0793"

good_samp <- scan(text = mail_text, what = character(), fill=T, sep = ",", strip.white = TRUE)
...
good_samp2 <- good_samp %>% setdiff(c("CSL161", "pM1835", "CSL211"))



#============== load quality prepared data ================

#__________ load SU2C new data and dataprep ___________
calls_suc <- fread("data/JB_Paper/clonalityCalls_broad_focal_SU2Cnew.txt")
...
CI95_suc <- fread("data/JB_Paper/allCalls_CN_CI_broad_focal_SU2Cnew.txt")


all_samp <- calls_suc$sample
good_samp %in% all_samp
# which false 
good_samp[!good_samp %in% all_samp]


# remove MM samples from the SU2C cohort
calls_ORIG <- calls_suc %>% filter(sample %in% good_samp2) %>% arrange(sample)
CI95_ORIG <- CI95_suc %>% filter(ID %in% good_samp2) %>% arrange(ID)
purity_ORIG <- purity_suc %>% filter(sample %in% good_samp2)  %>% arrange(sample)
...
for (b in 1:30) {
    ...
    dir.create("workfiles/Phylogic_comparison/", showWarnings = FALSE, recursive = TRUE)

    # save aggregated table with points
    write_tsv(expscorestab, paste0("workfiles/Phylogic_comparison/PhylComp_BOOT_",b,"_matches_results_ClonDiffPoints.txt"))
}

stopCluster(cl)
stopImplicitCluster()

# write_tsv(boot_data, "workfiles/Phylogic_comparison/PhylComp_BOOT-data.txt")