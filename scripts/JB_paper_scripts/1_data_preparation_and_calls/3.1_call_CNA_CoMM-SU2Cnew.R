...
################### BROAD AND FOCAL CALLS - CoMMpass dataset ###################
...
segm <- fread("data/CoMMpass_acs/dataprep_segments_CoMM_847.seg")
...
test <- res.gene$ID %>% unique


samples[!samples %in% test]
...
for ( i in 1:nrow(all_focal_df)) {
  ...
  CNvalues<- res.gene.extreme$BOB_CN %>% round(4)# extract the CN values 
  ...
  dfExport <- res.gene.extreme %>% mutate(gene = G_gene$hgnc_symbol, 
                                          CN_CI= paste0(BOB_CN %>% round(4),"|", seg_sigma %>% round(4) )) %>% 
...
genesCallsFinal <- genesCallsFinal %>% rename(BCL6 = "chr_3q_BCL6",
                                              BIRC2 = "chr_11q_BIRC2",
...
anno <- fread("data/CoMMpass_acs/CoMMpass_ABSOLUTE_tables_969samp.txt")

pur_tab <- anno %>% dplyr::select(ID= sample, purity)
...
for(i in samples){
  ...
  segments$CN_abs_dev <- abs(segments$BOB_CN - 2)
  ...
  for(a in ARMS){
    ...
    if(nrow(alt_seg)>0) {
      wm_CN <- weighted.mean(alt_seg$BOB_CN, w = alt_seg$width)
      wm_CI <- weighted.mean(alt_seg$seg_sigma, w = alt_seg$width)
      
    } else {
      wm_CN <- weighted.mean(seg_arm$BOB_CN, w = seg_arm$width)
...
res_armcalls_table2 <- res_armcalls_table %>% dplyr::select(-chr_13p, -chr_22p, -chr_21p )
...
# save
write_tsv(ALL_CALLS, "results/JB_Paper/calls/allCalls_CN_CI_broad_focal_CoMM-SU2Cnew.txt")
...
write_tsv(ALL_CLON_CALLS_def, "results/JB_paper/calls/clonalityCalls_broad_focal_CoMM-SU2Cnew.txt")
