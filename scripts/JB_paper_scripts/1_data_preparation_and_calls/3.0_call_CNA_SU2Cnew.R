...
segm <- fread("data/JB_Paper/dataprep_segments_SU2CNew_164.txt")
...
segm_arm <- BioNerds::Popeye2(segm %>% dplyr::rename(sample = ID), refGenome = "hg38")
segm_arm$ID <- segm_arm$sample

# import Purity file, needed to compute normal threshold for segments
anno <- readxl::read_xlsx("data/JB_Paper/DuttaAlberge_CleanClinicalRef_NOMRN.xlsx")

pur_tab <- anno %>% dplyr::select(ID= Reference_Pair, purity)
...
for(i in samples){
  ...
  for(a in ARMS){
    ...
    if(nrow(alt_seg)>0) {
      ...
      wm_CI <- weighted.mean(alt_seg$CI95_purCorr, w = alt_seg$width)
      
    } else {
      wm_CN <- weighted.mean(seg_arm$CN, w = seg_arm$width)
      wm_CI <- weighted.mean(seg_arm$CI95_purCorr, w = seg_arm$width)
...
# save
write_tsv(ALL_CALLS, "results/JB_Paper/calls/allCalls_CN_CI_broad_focal_SU2Cnew.txt" )
...
write_tsv(ALL_CLON_CALLS_def, "results/JB_Paper/calls/clonalityCalls_broad_focal_SU2Cnew.txt" )
