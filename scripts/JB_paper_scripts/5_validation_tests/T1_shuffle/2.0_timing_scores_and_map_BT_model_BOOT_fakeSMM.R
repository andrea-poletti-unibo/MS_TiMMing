...
outdir <- "results/JB_paper/def_aug23/validation_tests/T1_shuffle/Fake_SMM_boots/"
dir.create(outdir, showWarnings = F, recursive = T)

# read in the data
files <- list.files("workfiles/validation_tests/T1_shuffle/Fake_SMM_boots", full.names = T, pattern = "matches")
...
write_tsv(res_all, paste0(outdir, "/fakeSMM_timings_20boot.txt"))

res_all <- fread(paste0(outdir, "fakeSMM_timings_20boot.txt"))
...
write_tsv(res_all, "workfiles/validation_tests/T1_shuffle/Fake_SMM_boots/BTmodels_20BOOTS_fakeSMM.txt")
write_tsv(res_all_boots, "workfiles/validation_tests/T1_shuffle/Fake_SMM_boots/Aggregate_BTmodels_20BOOTS_fakeSMM.txt")
...
p1 <- res_all %>% ggplot(aes(x=timing, y = cna_f)) + 
  ...
  ylab("CNA") +
  ggtitle("Bradley-Terry CNAs timing estimates on fake-SMM cohort", subtitle = "20 bootstraps runs")
...
# save plot
ggsave(plot = p1, filename =  "plots/timing_maps/validation_tests/T1_shuffle/fakeSMM_timing_map_20BOOTS.png", width = 14, height = 12, units = "in")
