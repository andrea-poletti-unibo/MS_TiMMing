...
outdir <- "workfiles/validation_tests/T1_shuffle/Fake_NDMM_boots/"
...
# FINAL COHORT SMM n= 325 samples
export_samples_list <- data.frame(sample = calls_0$sample)



#============== load COMPASS prepared data ================

calls_0C <- fread("results/JB_paper/calls/clonalityCalls_broad_focal_CoMM-SU2Cnew.txt")

purity_file_C <- fread("data/CoMMpass_acs/CoMMpass_ABSOLUTE_tables_969samp.txt")
purity_0C <- purity_file_C %>% select(sample, purity) %>% filter(sample %in% calls_0C$sample)

CI95_0C <- fread("results/JB_paper/calls/allCalls_CN_CI_broad_focal_CoMM-SU2Cnew.txt")



#=============== merge COMPASS data with SMM&MGUS data ================

calls_M <- rbind(calls_0, calls_0C)

CI95_M <- rbind(CI95_0, CI95_0C)

purity_M <- rbind(purity_0, purity_0C)


#_________________ select N = 325 random samples for FAKE SMM&MGUS cohort _______________

set.seed(123)

fake_samples <- calls_M %>% pull(sample) %>% sample(847)
fake_samples %>% str_detect("MMRF") %>% table()

# FALSE  TRUE 
#   237   610 
237/847
# 28% of the samples are from SMM/MGUS cohort

calls_S <- calls_M %>% filter(sample %in% fake_samples)
CI95_S <- CI95_M %>% filter(ID %in% fake_samples)
purity_S <- purity_M %>% filter(sample %in% fake_samples)
...
calls_ORIG <- calls_S %>% arrange(sample)
CI95_ORIG <- CI95_S %>% arrange(ID)
purity_ORIG <- purity_S %>% arrange(sample)
...
for (b in 11:20) {
    ...
    write_tsv(expscorestab, paste0(outdir,"/Fake_NDMM_BOOT_",b,"_matches_results_ClonDiffPoints.txt"))
}

stopCluster(cl)

write_tsv(boot_data, paste0(outdir, "/Fake_NDMM_BOOT-data.txt"))