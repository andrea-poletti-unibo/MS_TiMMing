...
outdir <- "workfiles/validation_tests/T1_shuffle/Fake_SMM_boots/"
...
fake_samples <- calls_M %>% pull(sample) %>% sample(325)
fake_samples %>% str_detect("MMRF") %>% table()
# FALSE  TRUE
#    81   244
81/325
# 25% of the samples are from SMM/MGUS cohort
...
# for (b in 1:20) {
    ...
    write_tsv(expscorestab, paste0(outdir,"/Fake_SMM_BOOT_",b,"_matches_results_ClonDiffPoints.txt"))
}

stopCluster(cl)

write_tsv(boot_data, paste0(outdir, "/Fake_SMM_BOOT-data.txt")) 