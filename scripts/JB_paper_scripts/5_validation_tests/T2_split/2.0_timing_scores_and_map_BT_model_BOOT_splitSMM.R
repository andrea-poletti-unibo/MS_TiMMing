...
outdir <- "results/JB_paper/def_aug23/validation_tests/T2_split_SMM/"
dir.create(outdir, showWarnings = F, recursive = T)


################################## split A #####################################

workpath <- "workfiles/validation_tests/T2_split_SMM/split_SMM-a/"
...
write_tsv(res_all, paste0(outdir, "/splitSMM-a_timings_20boot.txt"))
res_all <- fread(paste0(outdir, "splitSMM-a_timings_20boot.txt"))
...
write_tsv(res_all, paste0(workpath, "/BTmodels_20BOOTS_splitSMM-a.txt"))

write_tsv(res_all_boots, paste0(workpath, "/Aggregate_BTmodels_20BOOTS_splitSMM-a.txt"))
...
p1 <- res_all %>% ggplot(aes(x=timing, y = cna_f)) + 
  ...
  ylab("CNA") +
  ggtitle("Bradley-Terry CNAs timing estimates on split-SMM cohort", subtitle = "20 bootstraps runs")

p1

dir.create("plots/timing_maps/validation_tests/T2_split/", showWarnings = F, recursive = T)
# save plot
ggsave(plot = p1, filename =  "plots/timing_maps/validation_tests/T2_split/splitSMM-a_timing_map_20BOOTS.png", width = 14, height = 12, units = "in")
...
workpath <- "workfiles/validation_tests/T2_split_SMM/split_SMM-b/"
...
write_tsv(res_all, paste0(outdir, "/splitSMM-b_timings_20boot.txt"))
res_all <- fread(paste0(outdir, "splitSMM-b_timings_20boot.txt"))
...
write_tsv(res_all, paste0(workpath, "/BTmodels_20BOOTS_splitSMM-b.txt"))

write_tsv(res_all_boots, paste0(workpath, "/Aggregate_BTmodels_20BOOTS_splitSMM-b.txt"))
...
p1 <- res_all %>% ggplot(aes(x=timing, y = cna_f)) + 
  ...
  xlab("Timing Estimate") +
  ylab("CNA") +
  ggtitle("Bradley-Terry CNAs timing estimates on split-SMM-b cohort", subtitle = "20 bootstraps runs")

p1

dir.create("plots/timing_maps/validation_tests/T2_split/", showWarnings = F, recursive = T)
# save plot
ggsave(plot = p1, filename =  "plots/timing_maps/validation_tests/T2_split/splitSMM-b_timing_map_20BOOTS.png", width = 14, height = 12, units = "in")
