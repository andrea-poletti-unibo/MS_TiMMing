...
outdir <- "workfiles/validation_tests/T2_split_NDMM/split_NDMM-a/"
...
for (b in 1:20) {
    ...
    write_tsv(expscorestab, paste0(outdir,"/split_NDMM-a_BOOT_",b,"_matches_results_ClonDiffPoints.txt"))
}

stopCluster(cl)

write_tsv(boot_data, paste0(outdir, "/split_NDMM-a_BOOT-data.txt"))
...
######################### TIMMING MODEL SPLIT B  #########################
######################### 20 bootstraps #########################


outdir <- "workfiles/validation_tests/T2_split_NDMM/split_NDMM-b/"
dir.create(outdir, showWarnings = FALSE, recursive = TRUE)
...
for (b in 1:20) {
    ...
    # save aggregated table with points
    write_tsv(expscorestab, paste0(outdir,"/split_NDMM-b_BOOT_",b,"_matches_results_ClonDiffPoints.txt"))
}

stopCluster(cl)

write_tsv(boot_data, paste0(outdir, "/split_NDMM-b_BOOT-data.txt"))